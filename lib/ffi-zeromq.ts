...
export class Context {
  ...
  constructor() {
    ...
    if (this._ptr === null) {
      ...
      const errno = zmq.zmq_errno()
      const errorMsg = zmq.zmq_strerror(errno)
      throw new Error(`Failed to create ZeroMQ context: ${errorMsg.toString()} (errno ${errno})`)
    }
  }
...
export class Socket {
  ...
  constructor(context: Context, type: number) {
    ...
    if (this._ptr === null) {
      ...
      throw new Error(`Failed to create ZeroMQ socket: ${errorMsg.toString()} (errno ${errno})`)
    }
  }
  ...
  setOption(option: number, value: Buffer | string | number, isTopic: boolean = false): void {
    if (!this._ptr) throw new Error('Socket is closed or invalid.')
    let optval: Buffer
  ...
  send(message: string | Buffer, flags: number = 0): number {
    if (!this._ptr) throw new Error('Socket is closed or invalid.')
    const messageBuffer = Buffer.isBuffer(message) ? message : Buffer.from(message + '\0')
    ...
    const rc = zmq.zmq_send(this._ptr, ptr(bufferToSend), bufferToSend.byteLength, flags)
  ...
  receive(bufferSize: number = 1024, flags: number = 0): string {
    ...
    const bytesReceived = zmq.zmq_recv(this._ptr, ptr(buffer), bufferSize, flags)
  ...
  receiveBinary(bufferSize: number = 1024, flags: number = 0): Buffer {
    ...
    return Buffer.from(Uint8Array.prototype.slice.call(buffer, 0, bytesReceived))
  }
  ...
  getOption(option: number): number {
    ...
    const rc = zmq.zmq_getsockopt(this._ptr, option, ptr(optval), ptr(optvallen))
